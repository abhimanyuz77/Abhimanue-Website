...
try {
    ...
    if (empty($fullname) || empty($age) || empty($email) || empty($phone) || empty($class) || empty($physics_level)) {
    ...
    $subject = "New Physics Foundation Signup: " . $fullname;
    
    $email_body = "New Physics Foundation Signup Application\n\n";
    $email_body .= "=================================\n\n";
    $email_body .= "Full Name: " . $fullname . "\n";
    $email_body .= "Age: " . $age . "\n";
    $email_body .= "Email: " . $email . "\n";
    $email_body .= "Phone: " . $phone . "\n";
    $email_body .= "Current Class: " . $class . "\n";
    ...
    // Email headers
    $headers = "From: Physics Foundation Signup <noreply@abhimanue.in>\r\n";
    $headers .= "Reply-To: " . $email . "\r\n";
    $headers .= "X-Mailer: PHP/" . phpversion();
    
    // Save to CSV file (as backup)
    $csv_file = '../data/physics-signups.csv';
    ...
    if ($file) {
        ...
        if (!$file_exists) {
            fputcsv($file, array('Timestamp', 'Full Name', 'Age', 'Email', 'Phone', 'Current Class', 'Physics Level', 'Message'));
        }
        
        // Add data
        fputcsv($file, array($timestamp, $fullname, $age, $email, $phone, $class, $physics_level, $message));
        fclose($file);
    }
    ...
    if (!$mail_sent) {
        error_log("Physics signup email failed to send for: " . $email);
    }
    
} catch (Exception $e) {
    $response['success'] = false;
    $response['message'] = $e->getMessage();
    error_log("Physics signup error: " . $e->getMessage());
}
